...
public class Juego extends InterfaceJuego
{
	...
	{  
		...
		//INICIAMOS los enemigos, sus lasers, asteroides y la nave astro y LO QUE HACE FALTA
	...
	int aleatorio = random.nextInt(50) + 380; //cada cuanto se crean los enemigos
	...
	private void comportamientoAstro() {
		...
		if (this.astro.getX() - this.astro.getRadio()>0 && (entorno.estaPresionada(entorno.TECLA_IZQUIERDA) || entorno.estaPresionada('a'))) {
			astro.moverIzquierda();
		}
				
		if (this.astro.getX() + this.astro.getRadio()<entorno.ancho() && (entorno.estaPresionada(entorno.TECLA_DERECHA) || entorno.estaPresionada('d'))) {
			astro.moverDerecha();
		}
